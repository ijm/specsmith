...
We'll not be concerned with AI systems whose primary value lies in open-ended conversation, creativity, or social participation. We'll focus on the narrower and already common class of coding AIs: the subset of AI tools used to generate, or verify technical artifacts such as source code, schematics, or technical documents. These systems operate entirely on authoritative inputs, typically written documents, and succeed or fail based on the precision and completeness of those inputs.
...
In the following sections we'll explore specifically what coding AIs need in tech documents, suggest some structure, and note some immediately useful implications. Let's start with a deeper look at the historical balance.
