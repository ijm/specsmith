...
This framing isn't arbitrary. Source code is itself a specification. A compiler doesn't "follow instructions"; it looks at what must be true after compilation and emits a program that satisfies that truth. This dual code versus specification view, sometimes described as proscriptive versus descriptive, or imperative versus declarative, is explored in depth in [more here](http://something.com/something), but the essential idea for us is that the high-level properties that make code compilable are the same properties that make documentation usable by a coding agent.
