...
This allows coding agents to participate in the iterative loop we already use: build something, verify it, inspect the failures, refine things, rinse, and repeat. The errors in each loop shrink, and a consistent desirable result emerges. Making this stable and predictable for an LLM clarifies how the components of each document influence model behavior and informed the taxonomy choices for our documents. We go into depth about this approach [in this article](http://localhost/otherarticle), but here we focus on the high-level principle of determinism and decidability.
